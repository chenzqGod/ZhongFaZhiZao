...
- (void)createUI {
    ...
    self.titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(14, 8, self.contentView.frame.size.width-14, 24)];
    self.titleLabel.font = [UIFont systemFontOfSize:17.0];
    self.titleLabel.text = @"默认标题";
    [self addSubview:self.titleLabel];

    self.mainImgView = [[UIImageView alloc]initWithFrame:CGRectMake(Margin, 8+CGRectGetMaxY(self.titleLabel.frame), self.contentView.frame.size.width-2*Margin, 272/2.0)];
    self.mainImgView.image = [UIImage imageNamed:@"消息-默认图"];
...
//    _lowbgdView.backgroundColor = [UIColor colorWithHexString:@"#244356"];
    ...
    _pimageView = [[UIImageView alloc]initWithFrame:CGRectMake(self.frame.size.width-Margin-15, (CGRectGetHeight(_lowbgdView.frame)-15)/2.0, 8, 15)];
